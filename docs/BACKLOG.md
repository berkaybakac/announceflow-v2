...
- [P0] **Agent Supervisor Restart Politikası (IoT hardening):** `BOOT_TOKEN_MAX_RETRIES` dolunca process'in graceful kapanması doğru; production cihazda process mutlaka supervisor altında (systemd veya Docker) yeniden ayağa kaldırılmalı. `Restart=always` / `--restart=always` kullanılırsa exit code bağımsız self-healing sağlanır. `on-failure` politikası seçilecekse retry-limit çıkışının non-zero code üretmesi ayrıca implemente edilmelidir. **Tetikleyici:** Faz 4 deploy manifestleri/service unit dosyaları hazırlanırken.
...
- [P0] ~~Download endpoint ACL zafiyeti (is_global/media_targets bazlı yetkilendirme): Device JWT ile medya indirme endpoint'i branch kapsamı doğrulaması yapmıyor. Şube bazlı erişim kısıtı ACL sözleşmesine göre zorunlu hale getirilecek.~~ **DONE (2026-03-23):** Download endpoint'e branch kapsam doğrulaması eklendi. `MUSIC` için `media_targets` ACL, `ANONS` için aktif `schedules` ACL zorunlu hale getirildi (`backend/repositories/media_repository.py`, `backend/routers/media.py`, `tests/test_media.py`).
- [P1] ~~Agent DB Volume Path (DevOps): Dockerization aşamasında agent.db kalıcılığını sağlamak için SQLite yolu relative (sqlite:///agent.db) halinden absolute bir volume path'ine (/data/agent.db) taşınacak ve settings.py buna göre güncellenecek.~~ **KISMİ (2026-03-02):** Local default `data/agent.db` olarak güncellendi (`agent/core/settings.py`). Docker absolute path (`/data/agent.db`) Faz 4'te env override ile tamamlanacak.
